...
Purpose
-------
Define a practical, team-friendly plan to build and validate a production model for the safety CCTV pipeline. This document is the source of truth for data collection, labeling, training, evaluation, and deployment steps.

Scope
-----
- Required classes (phase 1): PERSON, ZONE_INTRUSION, DANGER_ZONE_ENTRY.
- Optional classes (phase 2+): PPE, POSTURE, SMOKE, FIRE, HAZARD.
- Any new class beyond protocol v0.2 requires protocol update and agreement.
...
Deployment/Export
-----------------
- Export to ONNX for runtime use in `ai.vision.onnx_adapter`.
...
- v0.2 프로토콜 목록 외 클래스는 프로토콜 업데이트와 합의가 필요하다.
