...
1. Build: `docker build -t safety-ai .`
2. Run with snapshot volume: `docker run --rm -v /tmp/snapshots:/data/snapshots safety-ai`
3. Override config via env (example): `-e AI_LOG_LEVEL=DEBUG`
4. Note: container default is `--dry-run`. To post to backend, override the command:
   `docker run --rm -v /tmp/snapshots:/data/snapshots safety-ai python -m ai.pipeline`
...
1. 빌드: `docker build -t safety-ai .`
2. 스냅샷 볼륨 마운트하여 실행: `docker run --rm -v /tmp/snapshots:/data/snapshots safety-ai`
3. 환경 변수로 설정 오버라이드 (예시): `-e AI_LOG_LEVEL=DEBUG`
4. 참고: 컨테이너 기본값은 `--dry-run`임. 백엔드 전송을 하려면 커맨드를 명시적으로 지정:
   `docker run --rm -v /tmp/snapshots:/data/snapshots safety-ai python -m ai.pipeline`
