...
2. Model adapter implements `ModelAdapter` in the runtime repo:
   `safety-cctv-ai/src/ai/pipeline/model_adapter.py`.
...
--------------------------
1. Create `safety-cctv-ai/src/ai/vision/yolo_adapter.py` in the runtime repo.
...
PY
```
3. Use `models/yolov8n.onnx` as `ONNX_MODEL_PATH`.

Environment Variables
---------------------
1. `AI_MODEL_ADAPTER=ai.vision.yolo_adapter:YOLOAdapter`
...
2. 모델 어댑터는 런타임 레포의 `safety-cctv-ai/src/ai/pipeline/model_adapter.py`의
   `ModelAdapter`를 구현.
...
권장 어댑터 구성
---------------
1. 런타임 레포에 `safety-cctv-ai/src/ai/vision/yolo_adapter.py` 생성.
...
PY
```
3. `models/yolov8n.onnx`를 `ONNX_MODEL_PATH`로 사용.
